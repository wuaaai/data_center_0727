...
## 风险与缓解（code review 更新）

| 风险 | 影响 | 缓解 |
|------|------|------|
| c_metadata 大批量 UPDATE 死锁 | 中 | 分批 UPDATE，每批 500 条；迁移在事务中执行，失败自动 ROLLBACK |
| 迁移中断数据不一致 | 高 | **新增：** 迁移脚本写入前先 dry-run + 备份 c_metadata 快照；提供回滚脚本 |
| PostgreSQL 远端断连 kb 不可用 | 中 | **新增：** Task 1 增加本地 SQLite 缓存 kb 列表 + 定时同步 |
| 知识库选择器 200+ 项不可搜索 | 中 | **新增：** 选择器改为 el-select 可搜索模式；支持输入拼音首字母匹配 |
| 批量 Embedding API 返回顺序不确定 | 低 | 按 index 排序后再与 text 匹配 |
| Embedding API 不支持批量数组 | 中 | **新增：** 先验证 API 能力（发测试请求看响应），不支持则加入降级逻辑 |
| sync 模式阻塞线程池 | 中 | **新增：** sync 模式用独立线程池，不共享 FastAPI 默认线程池；单文件 5 分钟硬限制 |
| 目录扫描无权限校验 | 高 | **新增：** scan-directory 只允许白名单路径（环境变量 ALLOWED_SCAN_PATHS）；拒绝 ../ 和绝对路径 |
| 前端拖拽组件在旧浏览器上不兼容 | 低 | P1 优先级，先做文件选择方式 |
| 禁用文档向量空间浪费 | 低 | **新增：** 禁用只做标记不删向量；后续可增加定时清理任务 |

## 开放问题

- 是否需要为 kb 创建/编辑做权限校验（用户登录）？当前用 X-User-ID 头
- 批量入库的 COPY 模式是否需要回退方案（旧版 psycopg2）？
- sync 模式是否真的要支持？用户批量场景优先 async，sync 可降为 P1
- Embedding API (120.211.116.133:65325) 是否支持批量输入？需要先验证
