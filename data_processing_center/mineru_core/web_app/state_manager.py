...
# 持久化文件路径
_STORE_FILE = Path(__file__).resolve().parent.parent / "output" / "_jobs.json"
...
def create_job(filename: str, file_size: int, input_path: str) -> str:
    ...
    _create_job_inner(job_id, filename, file_size, input_path)
    return job_id


def create_job_with_id(job_id: str, filename: str, file_size: int, input_path: str) -> str:
    """使用预生成的 job_id 创建任务记录（避免 input_path 瞬时为空串）。"""
    _create_job_inner(job_id, filename, file_size, input_path)
    return job_id


def _create_job_inner(job_id: str, filename: str, file_size: int, input_path: str):
    """创建任务记录的内部实现。"""
    with _LOCK:
        _JOBS[job_id] = {
            "id": job_id,
...
def count_jobs(status: Optional[str] = None) -> int:
    ...
    with _LOCK:
        if status:
            return sum(1 for j in _JOBS.values() if j.get("status") == status)
        return len(_JOBS)
