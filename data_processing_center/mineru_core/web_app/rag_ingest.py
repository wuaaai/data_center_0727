...
# ---------- 项目根目录 ----------
_PROJECT_ROOT = Path(__file__).resolve().parent.parent  # mineru_core/
_WEB_APP_DIR = Path(__file__).resolve().parent           # mineru/web_app/
...
# ---------- 文档处理 ----------
def read_and_process_word(file_path, source_id=None, original_filename=None, region_code_from_job=None):
    try:
        ...
        for p_idx, parent_block in enumerate(parent_blocks):
            ...
            for c_idx, child_content in enumerate(child_blocks):
                ...
                metadata = {
                    ...
                    "region_code": region_code_from_job or region_mapping.get(file_name, "000000"),
...
def ingest(file_path, source_id=None):
    ...
    try:
        ...
        if jobs_file.exists():
            ...
            # Bug 41: 从 job 记录读取 region_code，替代不存在的 region_mapping.json
            if job and job.get("region_code") and job.get("region_code") != "000000":
                region_code_from_job = job["region_code"]
    except Exception:
        pass
    ...
    docs = read_and_process_word(file_path, source_id=source_id, original_filename=original_filename, region_code_from_job=region_code_from_job)
